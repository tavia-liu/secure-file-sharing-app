...
$ownerDir = $DIR . '/' . $owner;
$fullPath = $ownerDir . '/' . $filename;

$allowed = false;
$userFiles = file($SHARES_FILE, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
foreach ($userFiles as $userFile) {
    $parts = explode(':', $userFile); 
    if ($parts[0] === $owner && $parts[1] === $filename) {
        $recipients = explode(',', $parts[2]);
        if (in_array($username, $recipients, true)) {
            $allowed = true;
            break;
        }
    }
